...
- ~~Class M: `DEBUG` trap behaviour inside the shells Claude Code and Codex spawn (they run `bash -c`/`zsh -lc`; the wrapper approach covers this if traps do not).~~ **Resolved (2026-09-19, A7):** confirmed empirically that `bash -c`/`sh -c` is neither interactive nor login, so the trap/preexec hooks never attach; the PATH-first wrapper is shipped and covers it (`receipts record --wrapper`).
