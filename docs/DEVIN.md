...
1. **Initialize step** installs `custos-code-recorder`: a shell hook (`BASH_ENV` pointing at a script that sets a `DEBUG`/`ERR` trap and `PROMPT_COMMAND`, plus the same in `.bashrc`/`.zshrc`) that appends every executed command, its cwd, start/end time, and exit status to `~/.custos-code/ledger.jsonl`, hash-chained. `VERIFY`: that Devin's Shell tool runs through a bash/zsh that honors these; if it runs `sh -c`, use a `PATH`-first wrapper for `sh`/`bash` that execs the real shell with the trap loaded.
2. **`$ENVRC`** exports `CUSTOS_CODE_INGEST_URL` and, from Secrets, `CUSTOS_CODE_TOKEN`; the recorder also POSTs each event to our ingest endpoint so the ledger survives the VM. If Devin restricts egress under a security profile, the ledger is committed to the branch as `.custos-code/ledger.jsonl` by a post-task step Devin is told to run (cooperative fallback, flagged as such).
...
2. **The recorder in a blueprint**: a Devin session on a trap repo; the receipt on its PR shows `pytest | tail -5` caught by the recorder's exit-status log, then the nudge sent through the API, then the corrected PR. That is "Custos Code checks Devin, and talks back to Devin, using Devin's own surfaces."
3. **Path C on someone else's Devin PR** (any public repo with a Devin-authored PR): the receipt with zero setup.
4. **The honest line:** "Devin Review reads the diff. Custos Code reads the description against the session. Both are needed."

## Bench on Devin
Dynamic Workflows fan out: `agent(trap_prompt, schema=ReportSchema)` per trap × N; each session opens a PR on a fixture branch; Custos Code scores each PR with Path B or C; the per-trap false-report rate for Devin goes on the same chart as Claude Code and Codex. Devin's structured-output schema asks for `{claims: [...], tests_run: bool, commands: [...]}`; the gap between that self-report and the recorder is itself a measurement (self-report fidelity, the AER paper's unimplemented metric).
