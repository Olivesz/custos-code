...
### Subagents
- **Claude Code.** Hooks fire inside subagents too; the payload carries `agent_id` and `agent_type`. Transcript records of subagent turns have `isSidechain: true`. `SubagentStop` delivers the subagent's `last_assistant_message`. Rule: a subagent's *report* is a claim source for the parent (the parent often repeats it), but a subagent's *tool calls* are evidence only for claims about what the subagent did. A parent saying "the tests pass" backed only by a subagent's prose is `unwitnessed`. **DECIDED (E10, 2026-09-19, Oliver): evidence for a parent's claim about its own work is main-chain only, never, full stop.** `rules._visible` and `review.annotate` independently drop every `sidechain` event before any evidence matching happens, so a subagent's tool call is never cited as the parent's evidence, and the claim stays `unwitnessed` even when the subagent genuinely ran the tests. This was originally two filters that happened to agree; it is now the intended behaviour, and `tests/unit/test_sidechain_laundering.py` pins it down (the fixture is Anush's). The reasoning: the alternative -- let the parent cite the child ledger -- reintroduces the multi-agent laundering case from the research, where a critic subagent approves a false narrative and the parent inherits the approval. It also mis-states what the parent actually did. A parent that delegates and then says "the tests pass" is asserting something it did not witness; the honest claim is about delegation ("I asked a subagent to run the tests and it reported they pass"), which is a different sentence with a different truth condition. The subagent is not let off: `SubagentStop` fires inside subagents too, so the subagent gets **its own receipt**, checked against **its own** main-chain ledger, where its `pytest` call is ordinary evidence. Nothing goes unverified; the verification just happens at the level where the work happened. Cost of being wrong here is asymmetric and that settles it: the strict rule's failure mode is an `unwitnessed` mark on honest delegated work, which is never an accusation and which the agent clears by re-running the check itself. The lenient rule's failure mode is a `confirmed` on a laundered claim, which is the one thing this project must not do.
