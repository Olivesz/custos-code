...
| E4 | Tier 3 time budget in the Stop hook (10 s hook limit) | Anush | open | Rules block synchronously; re-run async, result posted as a follow-up. Spawn/poll/result-load implemented in `rerun.py` (`spawn_async`/`poll`/`load_result`), entry point `receipts _hook rerun-worker`; `_hook stop` itself still raises `NotImplementedError` at the point it needs `claims.extract`/`rules.check` to know which claims to spawn for. VERIFY: does Claude Code kill the Stop hook's process group on exit, and does a `start_new_session=True` child survive that? Unconfirmed; revisit if a result file is ever observed to go missing. |
| E5 | How to detect a wrapper script named like a runner (`./pytest`, PATH shadowing)? | Anush | open | Resolve binary path; require it outside the repo tree or in a known venv. Codex gives argv directly. Implemented in `parsers.py`: `wrap_command_for_resolution`/`strip_and_parse_trailer` (PreToolUse trailer, piggybacked on the same trick as E9's exit-code capture) and `is_trusted_runner_path` (repo-tree + venv/node_modules allowlist + committed-config override). VERIFY: does `command -v` inside the trailer see the same `PATH` the agent's shell actually used, e.g. after a mid-session `export PATH=...`? Shares E9's open question of whether the model sees the rewritten command. |
...
| E6 | Claim extraction: LLM structured output vs regex? | Oliver | decided (provisional) | **Regex default, LLM opt-in.** Measured 2026-09-19 on the 10 gold sessions with gpt-5-mini: 15 regex claims vs 79 LLM claims, ~9 overlapping. Most of the 70 LLM-only claims were not claims about work (section headings, design arguments, quoted terminal output). Recall up, precision down; a false accusation is the worst failure, so the loose extractor cannot be default. Revisit once the gold labels exist: tighten `EXTRACT_SYSTEM`, then re-measure precision/recall per claim type. |
