...
| E10 | Subagent laundering rule: parent claim backed only by a subagent's prose is unwitnessed; backed by the child ledger's event is confirmed | Oliver | open | Had a fixture, `tests/unit/test_sidechain_laundering.py`. Turns out the two independently-implemented consumers of `flags.sidechain` (`rules._visible`, `review.annotate`) already agree with each other but not with this row's own premise: a subagent's tool-call event is *never* cited as the parent's evidence, so the claim stays `unwitnessed` even when the subagent genuinely ran the tests -- stricter than "confirmed with the child ledger cited." Left `open` rather than flipping to `decided`: the current behavior is consistent and tested, but whether "never, full stop" or "confirmed, cited" is the right final call for the product is Oliver's, not something to declare settled just because two files happen to already agree. See docs/MECHANICS.md §1. |
...
| E12 | `cli.py`'s two report commands disagree on the default path: `check` (#35) now defaults to the one-call trap-aware `review.py` review whenever a model backend is configured, but `cost` (#17, PR #34) still always runs the old rules-ladder-plus-optional-judge path (`verdicts.run`) regardless of backend availability. Should `cost` follow `check`'s new default, stay on the ladder so its tier/method breakdown stays meaningful, or take a flag either way? | Oliver | open | Noticed while landing #34, which lives entirely in `cost.py`/`compress.py`/`eval/` (Anush's area); `cli.py`'s command wiring is Oliver's file, so left as a question here rather than changed unilaterally. `cost.compute()` itself is agnostic to which path produced its `VerdictRecord`s -- either shape works once this is decided. |
