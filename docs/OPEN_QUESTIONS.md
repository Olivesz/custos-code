...
| E9 | Claude Code stores no exit code for Bash. Summary parsing vs PreToolUse wrapping vs Tier 3 re-run? | Oliver | decided | All three, in that order. **VERIFY resolved by experiment (2026-09-19): the wrapped command's trailer IS visible to the model** — a live `pytest --version` under the hook returned `__RECEIPTS_BIN=…` and `__RECEIPTS_RC=0` in the agent's own tool output. So the stdout trailer must go: write it to a per-call file instead (issue #21). MECHANICS §2. |
