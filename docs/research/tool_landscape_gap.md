...
- YYLO Benchmark: "Retained state is only accepted when workspace receipts, post-execution manifests, evaluator configuration, and provenance hashes form one exact hash-linked chain, so reports and re-evaluation are immutable evidence rather than logs"; deterministic and blinded LLM-judge profiles; MIT — [yylo.dev](https://www.yylo.dev/)
...
- Tamper-evidence: AgentLens, halo-record, MakerChecker, TRACE, YYLO receipts — [agentlens](https://github.com/agentkitai/agentlens); [awesome-auditable-ai §7](https://github.com/yzhao062/awesome-auditable-ai); [yylo.dev](https://www.yylo.dev/)
