...
ORDER = [Verdict.CONTRADICTED, Verdict.QUALIFIED, Verdict.UNRECORDED, Verdict.UNWITNESSED,
         Verdict.CONFIRMED]
...
def _cited(record: VerdictRecord, ledger: Sequence[LedgerEvent]) -> str:
    ...
    for seq in record.evidence[:3]:
        ...
        detail = ""
        if event.input:
            detail = str(event.input.get("command") or event.input.get("file_path")
                         or event.input.get("path") or event.input.get("sha") or "")
...
def _coverage_line(coverage: Sequence[Coverage]) -> str:
    ...
    if unrequested:
        bits.append(f"{len(unrequested)} unrequested change{'s' if len(unrequested) > 1 else ''} "
...
def pr_comment(session: Session, claims: Sequence[Claim], verdicts: Sequence[VerdictRecord],
               ledger: Sequence[LedgerEvent], *, pr_url: str | None = None,
               coverage: Sequence[Coverage] | None = None, receipt_url: str | None = None) -> str:
    ...
    headline = (f"**Receipt for this PR's description** · {len(verdicts)} "
                f"claim{'s' if len(verdicts) != 1 else ''} · source: {session.agent} "
                f"session `{session.id[:12]}`")
    integrity = (f"ledger `{session.ledger_root_hash[:8]}` · {session.n_events} events · "
    ...
    for verdict in ORDER:
        for record in [r for r in verdicts if r.verdict is verdict]:
            ...
            lines.append(f"| {MARK[verdict][0]} **{verdict.value}** | "
    ...
    if counts[Verdict.CONTRADICTED]:
        lines += ["", "> A contradicted claim means the log carries positive evidence against it "
