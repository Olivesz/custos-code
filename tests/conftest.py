...
looked like real regressions. Once `make_backend()` started reading ~/.receipts/env (so the CLI
...
def _no_model_backend(monkeypatch: pytest.MonkeyPatch, tmp_path_factory: pytest.TempPathFactory) -> None:
    for var in ("OPENAI_API_KEY", "ANTHROPIC_API_KEY", "RECEIPTS_JUDGE_BACKEND"):
        monkeypatch.delenv(var, raising=False)
    missing = tmp_path_factory.getbasetemp() / "no-such-env-file"
    monkeypatch.setenv("RECEIPTS_ENV_FILE", str(missing))
