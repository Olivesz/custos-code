"""`receipts record --wrapper`: installs the PATH-first bash/sh wrapper (docs/ADAPTERS.md §4/§7)."""
...
def test_wrapper_and_install_together_write_the_path_line_into_the_rc_file(
    ...
    assert "receipts recorder (class M) wrapper" in rc
    ...
    assert (tmp_path / ".bashrc").read_text().count("receipts recorder (class M) wrapper") == 2  # open+close marker
